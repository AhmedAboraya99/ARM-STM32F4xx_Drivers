 ...
 *  GLOBAL FUNCTIONS
 *********************************************************************************************************************/
void MCA_GPIO_vInitPort(GPIO_Reg_t* GPIOx, GPIO_Cfg_t* Copy_GPIO_Cfg)
{
	/* Enable RCC GPIO port clock. */
	MCAL_RCC_vEnablePeriphClk(AHB1_BUS, GPIO_xSelectPort(GPIOx));
...
void MCA_GPIO_vSetPinCfg(GPIO_Reg_t* GPIOx, GPIO_Pin_t Copy_PinId, GPIO_Cfg_t* Copy_GPIO_Cfg)
{

	MCAL_RCC_vEnablePeriphClk(AHB1_BUS, GPIO_xSelectPort(GPIOx));
 	/* 1. Configure the GPIO port. */
 	ASSIGN_BITS(GPIOx->GPIO_MODER, Copy_PinId, TWO_BITS,Copy_GPIO_Cfg->GPIO_Mode);
  	/* 2. Configure the GPIO pull-up/pull-down. */
	...
	{
 			ASSIGN_BITS(GPIOx->GPIO_PUPDR, Copy_PinId, TWO_BITS,Copy_GPIO_Cfg->GPIO_Mode>>THREE_BITS);
	}
	/* 3. Configure the GPIO output. */
 	/* 4.  Configure the GPIO output speed. */
	/* Check mode is output */
	...
	|| Copy_GPIO_Cfg->GPIO_Mode == GPIO_MODE_AF_OUT_OD || Copy_GPIO_Cfg->GPIO_Mode == GPIO_MODE_AF_OUT_PP)
	{
			...
			ASSIGN_BITS(GPIOx->GPIO_OSPEEDR, Copy_PinId, TWO_BITS,Copy_GPIO_Cfg->GPIO_Speed);
	}


}
void MCA_GPIO_vDirectPinSetter(GPIO_Reg_t* GPIOx, GPIO_Pin_t Copy_PinId, Pin_State_t Copy_xPinVal)
{
...
u8 	MCA_GPIO_u8GetPinVal(GPIO_Reg_t* GPIOx, GPIO_Pin_t Copy_PinId)
{
	return (GET_BIT(GPIOx->GPIO_IDR, Copy_PinId));
}
void MCA_GPIO_vWritePort(GPIO_Reg_t* GPIOx, u32 Copy_u32PortVal)
{
	WRITE_REG(GPIOx->GPIO_ODR, Copy_u32PortVal);
}
u32 MCA_GPIO_u32ReadPort(GPIO_Reg_t* GPIOx)
{
	return READ_REG(GPIOx->GPIO_IDR);
}

GPIO_Select_t GPIO_xSelectPort(GPIO_Reg_t* GPIOx)
{
	u32 Local_PortOffset = 0;
	Local_PortOffset = GPIOx - GPIOA;
		return ((GPIO_Select_t)(Local_PortOffset/0x400));
